...
We are going to use a Hyper-V Server in Azure running nested virtualization to simulate our on-premise server.  At the completion of this lab you will cover the following:
...
5. In the **Migrate project** tab, select your Azure subscription, and then create a resource group named **AzureMig**.
6. In **Project Details**, specify **TIW** as the project name and click **Next**.
7. On the  **Select assessment tool** tab select **Azure Migrate: Server Assessment** and click **Next**.
8. In **Select migration tool** tab select **Skip adding a migration tool for now** and click **Next**.
9. In **Review + add tools**, review the settings and click **Add tools**.
10. Wait a few minutes for the Azure Migrate project to deploy. You'll be taken to the project page. If you don't see the project, you can access it from **Servers** in the Azure Migrate dashboard.

> *When you get an error that the deployment failed because of policy violation, why might that be?  Recall that in lab 3 you set a policy that required a specific tag and description and during setup of the Azure Migration project no tag data was entered.  You need to delete the policy assignment in order to continue.* 

11. In the Azure Portal click on **Microsoft Azure > Policy > Require tag and its value > Delete assignment**.
12. Repeat steps 1 thru 10.

## Set up the appliance VM

Azure Migrate Server Assessment runs a lightweight Hyper-V VM appliance.  This appliance performs VM discovery and sends VM metadata and performance data to Azure Migrate: Server Assessment.

To set up the appliance you:

1. Download a compressed Hyper-V VHD from the Azure portal.
2. Create the appliance, and check that it can connect to Azure Migrate Server Assessment.
3. Configure the appliance for the first time, and register it with the Azure Migrate project.
...
> *It will take upwards of 10 minutes to download the file.*
